...
void conv2(const double *wts, const double *bias, const double *iacts, double *oacts, size_t batch_sz) {
    ...
    for(int i30y = 0; i30y < IN3_Y; i30y+=2){
        for(int i30x = 0; i30x < IN3_X; i30x += 2){
            for (size_t n = 0; n < batch_sz; ++n) {
                for (size_t k = 0; k < CONV2_K; ++k) {
 ...
 register double volatile pee = wts[(k * CONV2_C * CONV2_S * CONV2_R) + (c * CONV2_S * CONV2_R) + (s * CONV2_R) + r];
  //register double volatile que = oacts[(n * CONV2_K * IN3_X * IN3_Y) + (k * IN3_X * IN3_Y) + (y * IN3_X) + x];
//   register double volatile arr = iacts[(n * CONV2_C * IN2_X * IN2_Y) + (c * IN2_X * IN2_Y) + ((y + s) * IN2_X) + (x + r)];
                                    }
                                }
                            }
                        }
